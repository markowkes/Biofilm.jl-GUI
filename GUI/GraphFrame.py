...
class GraphFrame(customtkinter.CTkFrame):
    ...
    def __init__(self, master, **kwargs):
        ...
        def draw():
            ...
            test = customtkinter.CTkLabel(text= "test", master = self.graph_frame).grid(row = 0, column = 0)
                        
            #Buttons:
            self.constant_button = customtkinter.CTkRadioButton(master = self.graph_frame, text= "constant", command = constant)
            self.constant_button.grid(row = 1, column = 1, padx = 16, pady = 30)
            self.periodic_button = customtkinter.CTkRadioButton(master = self.graph_frame, text= "periodic", command = periodic)
            self.periodic_button.grid(row = 2, column = 1, padx = 16, pady = 30)
            self.sin_button = customtkinter.CTkRadioButton(master = self.graph_frame, text= "sinusoidal", command = sin)
