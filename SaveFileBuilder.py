...
class SaveFileBuilder():
    ...
    def getSimulationParams(self):
        ...
        content = content + sim_param_string.format(self.params["title"].get(), 
                                                    self.params["run_time"].get(), 
                                                    self.params["tolerance"].get(), 
                                                    self.params["output_period"].get())
        return content
    ...
    def buildSin(self): 
        content = ""
        for solute in self.solutes_arr:
            line = "(t) -> {}\n\t\t\t   "
            line = line.format(solute.getInflowParams())
    ...
    def getParticulateParams(self):
        ...
        particulate_param_string = """
            ...
            Kdet =  {},\t\t# Particulates detachment coefficient
            srcX =  [{}],\t\t# Source of particulates
            mu = [{}],\n"""
        content = content + particulate_param_string.format(self.buildStringParams(particulate_params[0]),
                                                             self.buildFloatParams(particulate_params[1]), 
                                                             self.buildFloatParams(particulate_params[2]), 
                                                             self.buildFloatParams(particulate_params[3]),
                                                             self.params["kdet"].get(), 
                                                             self.buildSrcX(), 
                                                             self.buildMu())
        return content
    ...
    def getSoluteParams(self):
        ...
        content = content + solute_param_string.format(self.buildStringParams(solute_params[0]), 
                                                       self.buildSin(), 
                                                       self.buildFloatParams(solute_params[1]), 
                                                       self.buildFloatParams(solute_params[2]), 
                                                       self.buildYxs(self.params["yield_coefficients"]), 
                                                       self.buildFloatParams(solute_params[3]), 
                                                       self.buildFloatParams(solute_params[4]), 
                                                       self.buildSrcS())
        return content
    ...
    def getTankParams(self):
        ...
        content = content + tank_param_string.format(self.params["volume"].get(), 
                                                     self.params["surface_area"].get(), 
                                                     self.params["flowrate"].get(), 
                                                     self.params["gridpoints"].get(), 
                                                     self.params["initial_thickness"].get(), 
                                                     self.params["layer_thickness"].get())
        return content
    ...
    def getBiofilmParams(self):
        ...
        content = content + biofilm_param_string.format(self.params["gridpoints"].get(), 
                                                        self.params["initial_thickness"].get(), 
                                                        self.params["layer_thickness"].get())
        return content
