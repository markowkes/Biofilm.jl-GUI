...
class ScrollableObjectFrame(customtkinter.CTkScrollableFrame): 
    ...
    def drawFrame(self):
            row = len(self.frame_list)+1
            #print("ADDING NEW FRAME ON ROW: " + str(row))
            self.frame_list[-1].grid(row = row, column = 0, pady = 10, sticky = "w")
            self.update()
            #print(str(len(self.frame_list)))


    def deleteFrame(self, object):
        self.frame_list.remove(object)
        self.printHelper()


    def printHelper(self):
        message = "ScrollableObjectFrame- object type: {}, object count: {}, object list: {}"
        if self.is_solute:
            message = message.format("solute", str(len(self.frame_list)), self.frame_list)
        else:
            message = message.format("particulate", str(len(self.frame_list)), self.frame_list)
        print(message)
    ...
    def addEmptyFrame(self):
        ...
        if self.is_solute:
            new_frame = SoluteObjectFrame.ObjectFrame(self, frame_params)
        else:
             new_frame = ParticulateObjectFrame.ObjectFrame(self, frame_params)
        self.YxsHelper()
